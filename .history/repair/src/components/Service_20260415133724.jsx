        ...
        <div className="grid grid-cols-2 lg:grid-cols-6 gap-3 md:gap-4">
          {services.map((service) => (
            <div key={service.id} className="group cursor-pointer">
              {/* Image Section */}
              <div className="relative h-28 sm:h-32 md:h-36 overflow-hidden bg-gray-100 rounded-xl">
                <Image 
                  src={service.image} 
                  alt={service.title} 
                  fill 
              ...
              <div className="px-2 pb-2">
                <button
                  ...
                  className="w-full bg-indigo-600 text-white text-xs font-semibold py-1.5 rounded-lg hover:bg-indigo-700 transition-all"
                >
                  Book Now →
                </button>
              </div>
